$ ()->
    ...
    ## 多言語処理に関して。
    isASCII = (text)-> /^[\x00-\x7F]*$/.test text

    ## 英語を表示
    _languageSelect = (ascii)->
        ($ ".section-header").each (idx, elem)->
            $elem = $ elem
            if ascii == isASCII $elem.text()
                $elem.parent().show()
            else
                $elem.parent().hide()
    showEnglish = ()-> _languageSelect true
    showJapanese = ()-> _languageSelect false
    ...
    (()->
        ...
        $en = $ "<button>"
        $en.text "English"
        $en.click ()-> showEnglish()
        $en.appendTo $cont

        $jp = $ "<button>"
        $jp.text "日本語"
        $jp.click ()-> showJapanese()
        $jp.appendTo $cont
    )()
